...
const ImmersiaPaymentPage: React.FC = () => {
  ...
  const handleAdminLogin = async () => {
    try {
      ...
      // Fetch full role to get description (contains role_type + maxDiscount)
      let fullRole = role;
      try {
  ...
  const handlePOSPayment = async () => {
    ...
    try {
      setLoading(true);
      setStatus('Initiating payment on Immersia terminal...');
      const merchantRef = merchantReference || transactionId;
      const posAmountKobo = Math.round(finalAmount * 100);
      const terminalSerial = process.env.REACT_APP_TERMINAL_SERIAL_IMMERSIA;
      if (!terminalSerial) {
        throw new Error('Immersia terminal serial is not configured.');
      }
      ...
      const response = await axios.post(`${process.env.REACT_APP_BACKEND_URL}/v1/admin/moniepoint/transactions`, {
        ...
        checkoutPayload: {
          ...
          payment_method: 'Immersia_Moniepoint',
        },
      });
        ...
        <div className="payment-page-body">
          <div className="checkout-panel">
            ...
            <div className="checkout-actions">
              ...
              <button className="checkout-btn checkout-btn-secondary" onClick={() => setPaymentMethod('Immersia_CASH')}>
                Cash
              </button>
            </div>
            {paymentMethod === 'Immersia_CASH' && <div className="status-banner warn">Cash mode selected. Discounts are disabled.</div>}
          </div>
